...
export default function HomePage() {
  ...
  const handlePreference = (value) => setSelectedPreferences((current) => current.includes(value) ? current.filter((i) => i !== value) : [...current, value])
  ...
  const transformRecipe = async () => {
    ...
    try {
      ...
      const nextUsage = await api('/api/usage', { token }).catch(() => usage)
      setUsage(nextUsage)
    } finally {
      setLoading(false)
      setLoadingStep(0)
    ...
    try {
      const res = await api('/api/recipes/save', {
        ...
        body: {
          ...
          originalName: result.recipeName.replace(' | Functional Version', ''),
          originalInput: recipeText || description,
          inputMethod,
          healingDiet,
          preferences: selectedPreferences,
          mealType,
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-3xl bg-white p-5 shadow-card">
        ...
        <div className="mt-4 space-y-3">
          {inputMethod === 'paste' ? <textarea value={recipeText} onChange={(e) => setRecipeText(e.target.value)} className="min-h-44 w-full rounded-2xl border border-sage/30 p-3" placeholder="Paste ingredients and instructions here..." /> : <div className="grid gap-3 md:grid-cols-2"><select value={mealType} onChange={(e) => setMealType(e.target.value)} className="min-h-11 rounded-xl border border-sage/30 px-3">{['breakfast', 'lunch', 'dinner', 'snack', 'dessert'].map((type) => <option key={type}>{type}</option>)}</select><input value={description} onChange={(e) => setDescription(e.target.value)} className="min-h-11 rounded-xl border border-sage/30 px-3" placeholder="Include ingredients + preferences" /></div>}
          <div className="grid gap-3 md:grid-cols-2">
            <div><label className="mb-1 block text-sm font-semibold">Healing Diet Protocol</label><select value={healingDiet} onChange={(e) => setHealingDiet(e.target.value)} className="min-h-11 w-full rounded-xl border border-sage/30 px-3">{healingDiets.map((diet) => <option key={diet}>{diet}</option>)}</select></div>
            <div><label className="mb-1 block text-sm font-semibold">Additional Preferences</label><div className="grid grid-cols-2 gap-2 rounded-xl border border-sage/20 p-2">{preferences.map((pref) => <label key={pref} className="flex items-center gap-2 text-xs"><input type="checkbox" checked={selectedPreferences.includes(pref)} onChange={() => handlePreference(pref)} />{pref}</label>)}</div></div>
          </div>
          <button type="button" disabled={usage.used >= usage.limit} onClick={transformRecipe} className="min-h-11 rounded-xl bg-terracotta px-4 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">Transform with FxRecipe AI</button>
          <p className="text-sm text-charcoal/70">{usage.used} of {usage.limit} free transformations used this month</p>
          {usage.used >= usage.limit && (
            <div className="rounded-2xl border border-terracotta/30 bg-terracotta/10 p-3 text-sm">
              ...
              <div className="mt-2 flex gap-2"><input value={waitlistEmail} onChange={(e) => setWaitlistEmail(e.target.value)} placeholder="Enter email for Pro waitlist" className="min-h-11 flex-1 rounded-xl border border-sage/30 px-3" /><button className="min-h-11 rounded-xl bg-sage px-4 text-white" onClick={() => push('Added to Pro waitlist!')}>Join Waitlist</button></div>
            </div>
          )}
        </div>
      ...
      <RecipeOutput data={result} onSave={saveRecipe} onFavorite={() => push('Added to favorites!', 'success')} onShare={shareRecipe} onRemix={() => setResult(null)} savedId={savedId} authed={isAuthed} onPromptLogin={() => setShowAuthModal(true)} />
      ...
      <section className="grid gap-4 md:grid-cols-2">
        ...
        <article className="rounded-3xl bg-white p-5 shadow-card"><h3 className="text-xl font-bold">How it works</h3><ol className="mt-2 list-decimal space-y-1 pl-5 text-sm"><li>Paste or describe your recipe</li><li>Choose your healing protocol</li><li>Get your functional version instantly</li><li>Save, share, and remix</li></ol></article>
      </section>
