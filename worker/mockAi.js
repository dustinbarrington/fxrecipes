const RULES = [
  ...
  { match: /all-purpose flour|wheat flour/i, replacement: 'almond flour + tapioca starch', reason: 'Removes gluten/refined flour while preserving baking structure.' },
  { match: /white sugar|brown sugar|granulated sugar|powdered sugar|corn syrup/i, replacement: 'coconut sugar or monk fruit blend', reason: 'Lowers glycemic impact and removes refined sugar.' },
...
function parseDescription(description = '', mealType = 'meal') {
  ...
  const title = text ? `${mealType[0].toUpperCase()}${mealType.slice(1)} ${text.slice(0, 48)}` : 'Custom Meal'
...
function updateInstructions(instructions, changes) {
  ...
  return instructions.map((step) => {
    ...
    changes.filter((c) => c.action === 'SWAP').forEach((change) => {
      ...
      if (source && new RegExp(source, 'i').test(next)) {
        next = next.replace(new RegExp(source, 'ig'), change.replacement)
      }
    })
    return next
  })
...
export function transformWithMockAI(payload) {
  ...
  return {
    ...
    cookTime: '25 min',
    servings: '4 servings',
    summary: `Kept the original dish intent and updated ${changed.length} ingredient(s) to match selected functional criteria.`,
